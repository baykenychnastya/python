...
class InputInteger(Input):
    ...
    def readValue(self):
        value = readInteger(f'Enter {self.valueName}: ', 'Value must be integer')
        isValid = self.validate(value)
...
class InputDate(Input):
    ...
    def getDate(self):
        year = readIntegerWithCheck('Enter year : ', lambda x: 0 < x < 2021, 'Invalid data, try again ')
        month = readIntegerWithCheck('Enter month : ', lambda x: 0 < x < 13, 'Invalid data, try again ')
        day = readIntegerWithCheck('Enter day : ', lambda x: 0 < x < 31, 'Invalid data, try again ')

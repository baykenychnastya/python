...
class TaxFreeCollection(object):
    ...
    def sort(self, attr, taxFreesToSort=None):
        if taxFreesToSort is None:
            taxFreesToSort = self.taxFrees

        try:
            if attr == 'Company':
                taxFreesToSort.sort(key=lambda x: str(getattr(x, f"get_{attr}")()).lower(), reverse=False)
            else:
                taxFreesToSort.sort(key=lambda x: getattr(x, f"get_{attr}")(), reverse=False)
        except:
            print('Invalid data,try again ')
            self.sort(attr, taxFreesToSort)

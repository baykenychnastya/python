...
class TAX_FREE(object):
    ...
    def __init__(self, ID=None, Company=None, Country=None, vat_rate=None, date_of_purchase=None, vat_code=None,
        ...
        self.Country = Input(Country, 'Country (Italy or Germany or France)', [lambda value: VALIDATION.validateEnumValue(value, CountryEnum)])
        ...
        self.vat_code = Input(vat_code, "Vat code (in kind VA***_**_***)", [lambda x: VALIDATION.patternValidation(x, "^VA.{3,3}_.{2,2}_.{3,3}$")])
        self.date_of_tax_free_registration = InputDate(date_of_tax_free_registration, "Date of tax free registration", [VALIDATION.validateDateString, lambda x: VALIDATION.validateLowerDateInputFile(self.date_of_purchase, x, "Date of tax free registration must be more date of purchase")])
    ...
    @VALIDATION.validation_decorator_with_two_arguments(
        ...
        VALIDATION.validateLowerDate(datetime.strptime(date_of_purchase, '%Y-%m-%d'), datetime.strptime(x, '%Y-%m-%d'), "Date of tax free registration must be more date of purchase"))
